...
- **Deployed (Vercel):** the static Storybook can't run the addon, so a serverless function [`api/mcp.ts`](./api/mcp.ts) serves a docs-only MCP at `/api/mcp` (auto-routed by Vercel; `vercel.json` only configures the function + static output). It reads `storybook-static/mcp/components.json`, generated by [`scripts/mcp/build-metadata.ts`](./scripts/mcp/build-metadata.ts) as part of `yarn build-storybook`.
- Do not break the `build-storybook` → metadata chain; the deployed `/api/mcp` depends on the generated catalog. Not published in the npm package (`files` ships only `dist`).
