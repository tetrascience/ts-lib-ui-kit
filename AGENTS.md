...
Exposes the UI kit to AI coding agents (component lists, props/variants, usage examples)
...
- **Deployed (Vercel):** the static Storybook can't run the addon, so a serverless function [`api/mcp.ts`](./api/mcp.ts) serves a docs-only MCP at `/api/mcp`. It imports `api/_components.json`, which the bundler inlines into the function. That catalog is generated by [`scripts/mcp/build-metadata.ts`](./scripts/mcp/build-metadata.ts) as part of `yarn build-storybook`.
- [`vercel.json`](./vercel.json) is **required**: it pins `buildCommand` to `yarn build-storybook` (so the metadata step writes `api/_components.json` before the function is bundled) and declares the `api/mcp.ts` function. Without it the deployed function 404s — the build reverts to dashboard settings, `api/_components.json` is never generated, and the function's import fails to bundle.
- Do not break the `build-storybook` → metadata chain; the deployed `/api/mcp` depends on the generated `api/_components.json` (gitignored). Not published in the npm package (`files` ships only `dist`).
