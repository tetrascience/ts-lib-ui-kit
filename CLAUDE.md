...
├── server/          # Server-side utilities (multiple export subpaths)
...
- Unit tests: `*.test.ts` / `*.test.tsx` or `__tests__/` directories
- Storybook tests: `play` functions in `*.stories.tsx`, run via `@storybook/addon-vitest` + Playwright
