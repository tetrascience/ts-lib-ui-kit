...
function render(props: InteractiveScatterProps) {
  const container = document.createElement("div");
  document.body.appendChild(container);
  const root = createRoot(container);
  roots.push({ root, container });
  act(() => {
    root.render(<InteractiveScatter {...props} />);
  });
  return {
    container,
    rerender: (nextProps: InteractiveScatterProps) =>
      act(() => {
        root.render(<InteractiveScatter {...nextProps} />);
      }),
    unmount: () =>
      act(() => {
        root.unmount();
...
describe("InteractiveScatter rendering", () => {
  ...
  it("appends a custom className to the container", () => {
    ...
    expect(container.querySelector(".interactive-scatter.custom")).not.toBeNull();

    const plain = render({ data });
    expect(plain.container.firstElementChild?.className).toBe("interactive-scatter relative");
  });
