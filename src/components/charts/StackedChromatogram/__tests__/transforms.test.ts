...
import type { ChromatogramSeries, PeakAnnotation } from "../../Chromatogram";
...
describe("applyStackingTransform - overlay mode", () => {
  it("returns the input series unchanged", () => {
    ...
    const result = applyStackingTransform(series, undefined, "overlay", 10);
...
describe("applyStackingTransform - stack mode (first-on-bottom)", () => {
  it("shifts each series y values by index * stackOffset", () => {
    ...
    const result = applyStackingTransform(series, undefined, "stack", 10, "first-on-bottom");
  ...
  it("preserves original series objects (does not mutate)", () => {
    ...
    applyStackingTransform(series, undefined, "stack", 10);
  ...
  it("handles undefined annotations gracefully", () => {
    ...
    const result = applyStackingTransform(series, undefined, "stack", 10);
    expect(result.annotations).toHaveLength(0);
  });
  ...
  it("computes stacked yRange including annotation y values", () => {
    ...
    const result = applyStackingTransform(series, anns, "stack", 10);
...
describe("applyStackingTransform - stack mode (first-on-top)", () => {
  it("reverses stacking: last series gets no shift, first gets max shift", () => {
    ...
    const result = applyStackingTransform(series, undefined, "stack", 10, "first-on-top");
  ...
  it("default stackingOrder is first-on-bottom", () => {
    const series = [makeSeries([0, 5]), makeSeries([0, 5])];
    const resultDefault = applyStackingTransform(series, undefined, "stack", 10);
    const resultExplicit = applyStackingTransform(series, undefined, "stack", 10, "first-on-bottom");
