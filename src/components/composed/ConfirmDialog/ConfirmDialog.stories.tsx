...
const meta: Meta<typeof ConfirmDialog> = {
  title: "Design Patterns/Confirm Dialog",
...
const defaultArgs = {
  ...
  description: "This workspace will be archived and hidden from your dashboard. You can restore it later.",
...
export const DefaultInteractionTest: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Trigger button renders", async () => {
      expect(canvas.getByRole("button", { name: "Archive workspace" })).toBeInTheDocument();
    ...
    await step("Clicking trigger opens the dialog", async () => {
      ...
      expect(within(dialog).getByRole("heading", { name: "Archive workspace" })).toBeInTheDocument();
    });

    await step("Cancel and confirm buttons are present", async () => {
      expect(body.getByRole("button", { name: "Cancel" })).toBeInTheDocument();
      expect(body.getByRole("button", { name: "Archive" })).toBeInTheDocument();
    });
...
export const DestructiveInteractionTest: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Trigger opens destructive dialog", async () => {
      await userEvent.click(canvas.getByRole("button", { name: "Delete experiment" }));
      expect(body.getByRole("dialog")).toBeInTheDocument();
    });

    await step("Warning callout renders in dialog body", async () => {
      expect(body.getByText("This action cannot be undone.")).toBeInTheDocument();
    });
...
export const AsyncConfirmInteractionTest: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Trigger button renders", async () => {
      expect(canvas.getByRole("button", { name: "Delete dataset" })).toBeInTheDocument();
    });

    await step("Clicking trigger opens dialog", async () => {
      await userEvent.click(canvas.getByRole("button", { name: "Delete dataset" }));
      expect(body.getByRole("dialog")).toBeInTheDocument();
    });
