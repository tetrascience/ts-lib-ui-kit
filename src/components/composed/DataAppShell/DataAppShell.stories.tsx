...
export const HelpButtonAbsent: Story = {
  name: "Help Button — Absent",
  render: () => (
    <DataAppShell
      appName="APP"
      navGroups={htsNavGroups}
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("No help button when onHelpClick is omitted", async () => {
      // Only the mobile hamburger button should be in the top nav
      const topNav = canvasElement.querySelector("[data-slot='data-app-top-nav']");
      const buttons = within(topNav!).getAllByRole("button");
      // Mobile hamburger is the only button in top nav (help button absent)
...
// =============================================================================
// No optional slots — minimal shell
// =============================================================================

export const MinimalShell: Story = {
  name: "Minimal Shell (no optional slots)",
  render: () => (
    <DataAppShell
      appName="MIN"
      navGroups={[{ pages: [{ id: "home", label: "Home", icon: ClipboardList }] }]}
    >
      <div className="p-6">
        <p data-testid="content">Bare minimum shell</p>
      </div>
    </DataAppShell>
  ),
  play: async ({ canvasElement, step }) => {
    ...
    await step("Shell renders with only required props", async () => {
      expect(canvas.getByText("MIN")).toBeInTheDocument();
      expect(canvas.getByText("Home")).toBeInTheDocument();
      expect(canvas.getByTestId("content")).toBeInTheDocument();
    });

    await step("No workflow panel is rendered", async () => {
      expect(canvasElement.querySelector("[data-slot='data-app-panel-expanded']")).toBeNull();
      expect(canvasElement.querySelector("[data-slot='data-app-panel-collapsed']")).toBeNull();
    });

    await step("No user menu section at sidebar bottom", async () => {
      // No avatar or user name text
      expect(canvas.queryByText("EL")).not.toBeInTheDocument();
    });

    await step("No breadcrumbs rendered when prop omitted", async () => {
      // BreadcrumbList should be empty / not visible
      const nav = canvasElement.querySelector("[data-slot='data-app-top-nav']");
      expect(nav).toBeInTheDocument();
      // No separator slashes
      const separators = canvasElement.querySelectorAll("[data-slot='breadcrumb-separator']");
...
// =============================================================================
// Nav pages without icons (dot fallback)
// =============================================================================

export const PagesWithoutIcons: Story = {
  name: "Nav Pages Without Icons",
  render: () => (
    <DataAppShell
      appName="APP"
      navGroups={[{
        pages: [
          { id: "a", label: "Alpha", isActive: true },  // no icon → active dot
          { id: "b", label: "Beta" },                   // no icon → inactive dot
        ],
      }]}
      breadcrumbs={[{ label: "Alpha" }]}
    >
      <div className="p-6"><p>Content</p></div>
    </DataAppShell>
  ),
  play: async ({ canvasElement, step }) => {
    ...
    await step("Pages without icons show their labels", async () => {
      expect(canvas.getByText("Alpha")).toBeInTheDocument();
      expect(canvas.getByText("Beta")).toBeInTheDocument();
    });

    await step("Dot fallback indicators are rendered instead of icons", async () => {
      const rail = canvasElement.querySelector("[data-slot='data-app-sidebar-rail']");
      const dots = rail?.querySelectorAll(".rounded-full");
      // Each page without an icon gets a dot
      expect(dots?.length).toBeGreaterThanOrEqual(2);
    });

    await step("Active page dot uses primary colour", async () => {
      const rail = canvasElement.querySelector("[data-slot='data-app-sidebar-rail']");
      const activeDot = rail?.querySelector(".bg-primary");
...
// =============================================================================
// Data count pills — clickable variant
// =============================================================================

export const DataCountPillsClickable: Story = {
  name: "Data Count Pills — Clickable",
  render: () => {
    const dataCounts: DataCount[] = [
      { label: "INPUT",  count: 10000, variant: "outline", onClick: () => console.log("input") },
      { label: "Output", count: 4500,  variant: "primary", onClick: () => console.log("output") },
    ];
    return (
      <DataAppShell
        appName="APP"
        navGroups={htsNavGroups}
        headerActions={<DataCountPills dataCounts={dataCounts} />}
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Data count pills render with labels and formatted counts", async () => {
    ...
    await step("Arrow separator is shown between pills", async () => {
      const topNav = canvasElement.querySelector("[data-slot='data-app-top-nav']");
      expect(topNav?.textContent).toContain("→");
    });

    await step("Clickable pills render as button elements", async () => {
      // DataCountPills renders <button> when onClick is provided
      const topNav = canvasElement.querySelector("[data-slot='data-app-top-nav']");
      const pillButtons = within(topNav!).getAllByRole("button");
      // hamburger + pill buttons (INPUT + Output)
      const pillBtns = pillButtons.filter(
        (b) => b.textContent?.includes("INPUT") || b.textContent?.includes("Output")
      );
      expect(pillBtns).toHaveLength(2);
    });
  },
};
