...
const meta: Meta<typeof StatCard> = {
  title: "Design Patterns/Stat Card",
  component: StatCard,
  parameters: {
    layout: "centered",
  },
...
export const Default: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Trend icon is present", async () => {
      expect(canvas.getByText("+14%").closest("[data-slot='stat-card']")!.querySelector("svg")).toBeInTheDocument();
    });
  },
};
...
export const GridOfFour: Story = {
  render: () => (
    <div className="grid grid-cols-2 gap-4 w-[640px]">
      <StatCard label="Files Processed" value="12,480" delta="+14%" deltaLabel="vs. last 7 days" trend="up" />
      <StatCard label="Failed Jobs" value="23" delta="+8" deltaLabel="vs. last 7 days" trend="down" />
      <StatCard label="Avg Runtime" value="2m 14s" delta="-12%" deltaLabel="faster" trend="up" />
      <StatCard label="Pipeline Success Rate" value="98.1%" delta="0%" deltaLabel="no change" trend="neutral" />
    </div>
  ),
  parameters: {
