import { BellIcon, CircleCheckIcon, InfoIcon, TriangleAlertIcon, XIcon } from "lucide-react";
...
export const InlineAlerts: Story = {
  render: () => (
    <div className="flex w-[480px] flex-col gap-3">
      <Alert variant="info">
        ...
        <AlertDescription>
          The Benchling ELN connector is now in beta. Enable it from your workspace settings.
        </AlertDescription>
      </Alert>
      <Alert variant="positive">
        <CircleCheckIcon />
        <AlertTitle>Pipeline JOB-9142 completed</AlertTitle>
        <AlertDescription>All 6,200 records were processed successfully and are ready for review.</AlertDescription>
      </Alert>
      <Alert variant="warning">
        <TriangleAlertIcon />
        <AlertTitle>Storage at 87% capacity</AlertTitle>
        <AlertDescription>Export or archive older datasets to avoid pipeline interruptions.</AlertDescription>
      </Alert>
      <Alert variant="destructive">
        <TriangleAlertIcon />
        <AlertTitle>API key expired</AlertTitle>
        <AlertDescription>Pipelines using the legacy key have been paused. Rotate your key to resume.</AlertDescription>
      </Alert>
    </div>
  ),
  parameters: {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Alert titles are visible", async () => {
      ...
      expect(canvas.getByText("Pipeline JOB-9142 completed")).toBeInTheDocument();
  ...
  render: () => (
    <div className="flex w-[420px] flex-col gap-3 rounded-xl border bg-background p-4">
      <p className="text-sm text-muted-foreground">
        Trigger each toast variant to preview Sonner styling with TDP-flavored content.
      </p>
        ...
        <Button
          ...
          onClick={() => {
            ...
            toast.warning("Retry scheduled", {
              description: "Connector reported a transient timeout.",
            });
          }}
        >
          Warning
...
export const AlertWithAction: Story = {
  render: () => (
    <div className="w-[480px]">
      <Alert>
        ...
        <AlertDescription>Your Q2 pipeline summary has been compiled and is ready to download.</AlertDescription>
        <AlertAction>
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Action button renders in alert", async () => {
      expect(canvas.getByRole("button", { name: "View report" })).toBeInTheDocument();
    });
  },
};
...
export const ContextualUsage: Story = {
          ...
          <CardContent className="flex flex-col gap-3">
            {showWarning ? (
              <Alert variant="warning">
                ...
                <AlertDescription>
                  One downstream connector reported a transient error. Retry to resume.
                </AlertDescription>
                <AlertAction>
                  <Button
            ...
            <div className="rounded-lg border p-3 text-sm text-muted-foreground">
              Records processed: <span className="font-medium text-foreground">4,820 / 6,200</span>
            </div>
          </CardContent>
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Card with warning alert renders", async () => {
      ...
      expect(canvas.getByRole("button", { name: "Dismiss alert" })).toBeInTheDocument();
    });
    ...
    await step("Action buttons are present", async () => {
      expect(canvas.getByRole("button", { name: "Retry pipeline" })).toBeInTheDocument();
      expect(canvas.getByRole("button", { name: "View logs" })).toBeInTheDocument();
    });

    await step("Clicking retry shows success toast", async () => {
      await userEvent.click(canvas.getByRole("button", { name: "Retry pipeline" }));
      const body = within(canvasElement.ownerDocument.body);
      await waitFor(() => {
        expect(body.getAllByText("Pipeline restarted").length).toBeGreaterThan(0);
      });
    });
  },
};
