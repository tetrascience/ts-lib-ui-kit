...
describe("TelemetryProvider metrics wiring", () => {
  ...
  test("counter reaches the reader — the instrument the bindings had omitted", async () => {
    // REGRESSION. useTetraEvents returned gauge, histogram and upDownCounter but
    // NOT counter, so a React data app could not emit the most basic instrument
    // of the four while Node and Python apps could. The core client and the
    // facade both had it; only the hook's returned object left it out, which is
    // why nothing here caught it — this file's own header described the surface
    // as gauge/histogram/upDownCounter.
    //
    // Found by the bindings' e2e against predev5: `events.counter is not a
    // function`, thrown in 14ms.
    let record: (() => void) | undefined;
    function App() {
      const { counter } = useTetraEvents();
      record = () => {
        counter("app.errors", 1, { "exception.type": "TypeError" });
        counter("app.errors", 2);
      };
      return null;
    }
    ...
    const errors = pointsFor("app.errors");
    expect(errors.length).toBeGreaterThan(0);
    // A monotonic Sum: both increments land on one series, totalling 3.
    const total = errors.reduce((sum, p) => sum + Number(p.value ?? 0), 0);
    expect(total).toBe(3);
  });
