...
describe("getTdpBaseUrlFromReferrer", () => {
  ...
  it("preserves org slug path prefix before route", () => {
    Object.defineProperty(document, "referrer", {
      value:
        "https://tetrascience-uat.com/customer-org/data-workspace/app/123",
    });
    expect(getTdpBaseUrlFromReferrer()).toBe(
      "https://tetrascience-uat.com/customer-org",
    );
  });
...
describe("getTdpBaseUrlFromApiEndpoint", () => {
  it("strips api. subdomain from hostname", () => {
    expect(getTdpBaseUrlFromApiEndpoint("https://api.tetrascience.com/v1")).toBe(
      "https://tetrascience.com",
    );
  });

  it("strips api. from UAT environment", () => {
    expect(
      getTdpBaseUrlFromApiEndpoint("https://api.tetrascience-uat.com/v1"),
    ).toBe("https://tetrascience-uat.com");
  });

  it("handles URL without api. prefix", () => {
    expect(
      getTdpBaseUrlFromApiEndpoint("https://tetrascience.com/v1"),
    ).toBe("https://tetrascience.com");
  });

  it("returns null for empty string", () => {
    expect(getTdpBaseUrlFromApiEndpoint("")).toBeNull();
  });

  it("returns null for malformed URL", () => {
    expect(getTdpBaseUrlFromApiEndpoint("not-a-url")).toBeNull();
  });
});

describe("buildTdpUrl", () => {
  it("joins base URL and path", () => {
    expect(buildTdpUrl("https://tetrascience.com", "/file/abc-123")).toBe(
      "https://tetrascience.com/file/abc-123",
    );
  });
  ...
  it("handles base URL with org prefix", () => {
    ...
    ).toBe("https://tetrascience.com/my-org/file/abc-123");
  });
...
describe("navigateToTdpUrl", () => {
  ...
  it("uses postMessage when inside an iframe", () => {
    ...
    expect(postMessageSpy).toHaveBeenCalledWith(
      { type: "navigate", path: "/my-org/file/abc?v=1#section" },
      "*",
    );
...
describe("tdpPaths", () => {
  ...
  it("artifact returns correct path", () => {
    expect(tdpPaths.artifact("ids", "common", "my-schema")).toBe(
      "/artifacts/ids/common/my-schema",
    );
  });
});
